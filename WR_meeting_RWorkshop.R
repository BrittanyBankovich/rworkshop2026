...
#To store objects, we use vectors. We establish a vector with a "<-". We can also store matrices, lists, data frames, etc.
...
#To quote or not to quote? Rules of thumb--use quotes with strings/text, do NOT use quotes when referencing objects (like calculation) or functions (like tibble())
...
#4 Pattern matching functions. These recognize four engines of pattern description. The most common is regular expressions (regex), but there are three other tools.
...
# dimensions
# First, let's store the plot as an object
...
# Say the journal you're submitting to wants figures to be .png, 180 mm wide
ggsave("figure1.png", plot = penguin_figure_for_ms,  width = 180, height = 180, units="mm", dpi = 300) 
